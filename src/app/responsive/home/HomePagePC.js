...
function HomePagePC() {
  ...
  useEffect(() => {
    if (!textControls || !ref.current) return
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) textControls.start('visible')
  ...
  useEffect(() => {
    if (!contactControls || !contactRef.current) return
    const observer = new IntersectionObserver(
      ([entry]) => {
  ...
  useEffect(() => {
    if (!productControls || !productRef.current) return
    const observer = new IntersectionObserver(
      ([entry]) => {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden relative">
      <motion.div animate={controls} className="relative overflow-hidden">
                ...
                <motion.div key={index} custom={index} variants={fadeUp} className="flex-1 flex flex-col items-center">
                  <div className="w-full h-4/5 rounded-2xl shadow-xl hover:shadow-2xl border border-gray-200 overflow-hidden flex flex-col transition-transform transform hover:-translate-y-2">
                    ...
                    <div className="bg-white p-4 flex justify-center items-center border-t border-gray-200">
                      <motion.span custom={index} variants={{ hidden:{opacity:0,y:20}, visible:{opacity:1,y:0,transition:{delay:index*0.2,duration:0.6,ease:'easeOut'}}}} className="text-gradient-to-r from-blue-400 via-lightBlue to-blue-600 text-3xl md:text-3xl font-bold uppercase tracking-widest text-center drop-shadow-xl">
                        {item.title}
                      </motion.span>
                    </div>
                  </div>
        ...
        {/* SECTION 4 - Contact */}
        <section ref={contactRef} className="h-screen w-full overflow-hidden">
