...
export default function ShoppingCart({
  ...
  const overlayOpen =
    propOverlayOpen !== undefined ? propOverlayOpen : internalOverlayOpen;
  const setOverlayOpen =
    propSetOverlayOpen !== undefined ? propSetOverlayOpen : setInternalOverlayOpen;
    ...
    <>
      ...
      >
        <motion.button
          ...
          className="relative w-15 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-lg"
        >
          {totalItems > 0 && (
      ...
      <AnimatePresence>
        {(showCartContent &&
          (overlayOpen || typeof window !== "undefined" && window.innerWidth >= 768)) && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              {/* Close Button */}
              <button
                onClick={() => setShowCartContent(false)}
